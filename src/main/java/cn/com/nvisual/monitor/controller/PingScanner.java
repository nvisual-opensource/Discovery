...
import cn.com.nvisual.monitor.model.*;
import cn.com.nvisual.monitor.service.*;
...
//            ipCreatNode.put("height",50d);
    ...
    //lldp发现纳管转发接口
    @RequestMapping(value = "/monitor/lldp_creact", method = RequestMethod.POST)
    public Object creactNodeBylldp(@RequestBody LldpReq lldpReq
            , HttpServletResponse httpServletResponse) {
        Map ret = new HashMap();
        List<LldpData> lldpData = lldpReq.getLldpData();
        Long diagramId = lldpReq.getDiagramId();
        String nvisualDeviceUrl = appConfig.getnVisual()+"/diagramApi/wapi/v1/lldp_topology_map?diagramId="+diagramId;

        String token = tokenService.getToken();
        Object returnData = auditHistoryService.postJSONArrayToken(nvisualDeviceUrl, token, lldpData);

        return returnData;
    }

    //lldp发现纳管转发接口2
    @RequestMapping(value = "/monitor/lldp_creact2", method = RequestMethod.POST)
    public Object creactNodeBylldp2(@RequestBody LldpReq lldpReq
            , HttpServletResponse httpServletResponse) {
        Map ret = new HashMap();
        List<LldpData> lldpData = lldpReq.getLldpData();
        Long diagramId = lldpReq.getDiagramId();
        String nvisualDeviceUrl = appConfig.getnVisual()+"/diagramApi/wapi/v1/lldp_topology_map2?diagramId="+diagramId;
